...
int head[SIZE], ver[SIZE * 2], Next[SIZE * 2];//head[u]代表与u直接相邻的第一个节点的序号。这个实际上就是链式前向星。
int dfn[SIZE], low[SIZE], c[SIZE];//dfn[u]代表节点u对应的时间戳，low[u]代表节点u的追溯值。
int n, m, tot, num, dcc, tc;//n个节点m条边。
bool bridge[SIZE * 2];//bridge[i]判断序号为i的边是否为桥。
